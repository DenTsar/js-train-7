...
function createAndCompareTwoSymbols(value) {
  ...
  const symba1 = Symbol(value);
  const symba2 = Symbol(value);
  return symba1 == symba2;
}
...
function useSymbolIterator(obj) {
  ...
  for (let num of obj) {
    result = result + num + ", ";
    // + ", " - використано для наглядності, розділення елементів масиву при виведені в консоль
    // як вказано в умові "//Виведе [ 1, 2, 3, 4, 5, 6 ]"
  }
  return result;
}
...
// ********************************************  хоча в дaному випадку проітерувати об'єкт по `name` простіше так
// let mas = [];
// for (let num of myObject.category) {
//   for (let item in num) {
//     if (item == `name`) {
//       mas = mas + num.name + " ,";
//     }
